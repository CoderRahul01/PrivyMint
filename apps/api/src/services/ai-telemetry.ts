...
export class AiTelemetryService {
  public calculateDauMetrics(): DauMetrics {
    ...
    users.forEach((u) => {
      const userHoldings = db.getHoldingsByCommitment(u.commitment);
  ...
  public generateAiTelemetryReport(): AiTelemetryReport {
    const dauMetrics = this.calculateDauMetrics();
    const offerings = db.getOfferings();
    ...
    return {
      ...
      operationalInsights: [
        ...
        {
          level: 'WARNING',
          title: 'Preprod RPC Fallback Triggered 4 Times',
          description: 'Preprod network RPC latency exceeded 2000ms, fallback ZK sandbox mode engaged cleanly.',
